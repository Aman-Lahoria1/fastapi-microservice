...
class ProfileRepository:
    @staticmethod
    async def get_by_auth_user_id(db: AsyncSession, auth_user_id: int) -> UserProfile | None:
        result = await db.execute(select(UserProfile).where(UserProfile.auth_user_id == auth_user_id))
        return result.scalar_one_or_none()

    @staticmethod
    async def upsert_from_registered_event(db: AsyncSession, user_id: int, email: str) -> UserProfile:
